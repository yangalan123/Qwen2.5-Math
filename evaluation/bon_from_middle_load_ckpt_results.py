...
from collect_majority_results import calculate_model_performance, plot_multi_model_comparison
...
def process_results(result_file):
    ...
    with open(result_file, "r", encoding='utf-8') as f:
        for line in f:
            ...
            for code, pred, score, output_token_num, loglik in zip(codes, preds, scores, output_token_nums, logliks):
                result = {
                    "code": code,
                    "pred": pred,
                    "score": score,
                    ...
                    "prompt_token_nums": prompt_token_nums[0],
                    "logliks": loglik,
                    "gt": gt,
                    "answer": answer
                }
...
if __name__ == '__main__':
    ...
    for model_dir in model_dirs:
        ...
        all_tasks = dict()
        all_original_data_idx_pred_res = dict()
        for result in all_individual_results:
            ...
            if data_idx not in all_original_data_idx_pred_res:
                all_original_data_idx_pred_res[data_idx] = {'pred': [], "score": []}
            all_original_data_idx_pred_res[data_idx]['pred'].append(pred)
            all_original_data_idx_pred_res[data_idx]['score'].append(score)
        prompts = []
        ...
        if os.path.exists(checkpoint_filename):
            ...
            print(f"Checkpoint file: {checkpoint_filename} not exists, terminated")
            exit()
        codes = []
        ...
        all_samples = []
        remainder_num = len(results) % (args.n_sampling * args.prefix_num_per_task)
        if remainder_num > 0:
            results = results[:-remainder_num]
        for i, sample in enumerate(linearized_all_tasks):
            if i * args.n_sampling >= len(results):
                break
            ...
            # if data_idx not in sample_mapping:
            #     sample_mapping[data_idx] = []
            new_sample = copy.deepcopy(sample)
            new_sample.update({"code": code, "pred": preds, "report": reports, "logliks": loglik_piece,
                               "original_sample": sample,
                               "token_ids_num": token_ids_num_piece, "entropies": entropies_piece})
            # sample_mapping[data_idx].append([sample, new_sample])
            new_sample.pop("score")
            # sample_mapping[data_idx].append(new_sample)
        ...
        if os.path.exists(temp_filename):
            all_samples = torch.load(temp_filename)
        else:
            ...
            torch.save(all_samples, output_filename+".temp")

        # save outputs
        # save_jsonl(all_samples, output_filename)
        #torch.save(all_samples, output_filename+".temp")
        sample_mapping = dict()
        original_sample_mapping = dict()
        for sample in all_samples:
            data_idx = sample['data_idx']
            if data_idx not in sample_mapping:
                sample_mapping[data_idx] = {"pred": [], "score": []}
                original_sample_mapping[data_idx] = {'pred': [], 'score': []}
            # original_sample_mapping[data_idx].append(sample['original_sample'])
            original_sample_mapping[data_idx]['score'].append(sample['original_sample']['score'])
            original_sample_mapping[data_idx]['pred'].append(sample['original_sample']['pred'])
            sample_mapping[data_idx]["score"].extend(sample['score'])
            sample_mapping[data_idx]["pred"].extend(sample['pred'])
        n_values = [1, 3, 8, 16]
        bootstrap_num = 100
        results_by_n = {n: [] for n in n_values}
        results_by_n_original = {n: [] for n in n_values}
        all_data_idx = list(sample_mapping.keys())
        for data_idx in all_data_idx:
            # original_preds = original_sample_mapping[data_idx]['pred']
            # original_scores = original_sample_mapping[data_idx]['score']
            original_preds = all_original_data_idx_pred_res[data_idx]['pred']
            original_scores = all_original_data_idx_pred_res[data_idx]['score']
            original_pred_to_score = dict()
            for pred, score in zip(original_preds, original_scores):
                if pred in original_pred_to_score:
                    assert original_pred_to_score[pred] == score
                original_pred_to_score[pred] = score
            preds = sample_mapping[data_idx]["pred"]
            scores = sample_mapping[data_idx]['score']
            pred_to_score = dict()
            for pred, score in zip(preds, scores):
                if pred in pred_to_score:
                    assert pred_to_score[pred] == score
                pred_to_score[pred] = score
            for n in n_values:
                majority_scores = []
                majority_scores_original = []
                for j in range(bootstrap_num):
                    #if n > len(original_preds):
                        #print(f"maj@{n} exceeds the maximum pred we can have for original predictions")
                        #break
                    if n > len(preds):
                        print(f"maj@{n} exceeds the maximum pred we can have for from-middle predictions")
                        break
                    original_segment_results = random.sample(original_preds, n)
                    majority_pick = Counter(original_segment_results).most_common(1)[0][0]
                    majority_score = original_pred_to_score[majority_pick]
                    # results_by_n_original[n].append(majority_score)
                    majority_scores_original.append(majority_score)
                    segment_results = random.sample(preds, n)
                    majority_pick = Counter(segment_results).most_common(1)[0][0]
                    majority_score = pred_to_score[majority_pick]
                    majority_scores.append(majority_score)
                results_by_n[n].append(majority_scores)
                results_by_n_original[n].append(majority_scores_original)
        # for data_idx in all_original_data_idx_pred_res:
        #     original_preds = all_original_data_idx_pred_res[data_idx]['pred']
        #     original_scores = all_original_data_idx_pred_res[data_idx]['score']
        #     original_pred_to_score = dict()
        #     for pred, score in zip(original_preds, original_scores):
        ...
        #             majority_score = original_pred_to_score[majority_pick]
        #             majority_scores.append(majority_score)
        #         results_by_n_original[n].append(majority_scores)

        performance_data = calculate_model_performance(results_by_n)
        original_performance_data = calculate_model_performance(results_by_n_original)
        visualization_data = {
            "Normal Majority Voting": original_performance_data,
            f"Majority Voting (after {args.prefix_token_num} tokens)": performance_data
        }
        print("original performance data")
        print(original_performance_data)
        print("bon from middle")
        print(performance_data)
        visualization_dir = os.path.join(DEFAULT_VISUALIZATION_DIR, f"bon_from_middle_{args.prefix_token_num}")
        plot_multi_model_comparison(args, visualization_data, args.temperature, args.top_p, visualization_dir=visualization_dir)
        print("visualization saved at: {}".format(visualization_dir))
