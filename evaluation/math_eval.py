...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=-1,
        help="Checkpoint frequency for vllm outputs.",
    )
    parser.add_argument(
        "--gpu_memory_utilization",
        type=float,
        default=0.7,
        help="(for vllm) gpu memory utilization.",
    )
    ...
    if "global_step_" in args.model_name_or_path:
        ...
        try:
            args.annealed_sampling_global_step = int(
                re.search(r"global_step_(\d+)", args.model_name_or_path).group(1)
            )
        except:
            print(
                f"Warning: global_step_x is part of the path, but cannot find the global_step, use default value {args.annealed_sampling_global_step}"
            )
    return args
...
def annealed_sampling_processor(
    ...
    if decay_mode == "global_step":
        current_temp = stability_temp + (exploration_temp - stability_temp) * np.exp(
            -global_step / decay_freq
        )
    elif decay_mode == "token_length":
        current_temp = stability_temp + (exploration_temp - stability_temp) * np.exp(
            -len(token_ids) / (20 * decay_freq)
        )
    elif decay_mode == "both":
        _exploration_temp = exploration_temp * np.exp(-global_step / decay_freq)
        current_temp = stability_temp + (_exploration_temp - stability_temp) * np.exp(
            -len(token_ids) / (20 * decay_freq)
        )
    elif decay_mode == "both_v_1_5":
        _decay_freq = min(decay_freq + 5 * global_step, 2000)
        current_temp = stability_temp + (exploration_temp - stability_temp) * np.exp(
            -len(token_ids) / (20 * _decay_freq)
        )
        ...
        current_temp = (
            1 + exploration_temp - np.exp(len(token_ids) / (20 * _decay_freq))
        )
        ...
        current_temp = exploration_temp + (stability_temp - exploration_temp) * np.exp(
            -global_step / decay_freq
        )
...
def run_llm_via_vllm(args, llm, prompts, stop_words=None, checkpoint_filename=None):
    ...
    entropies = []
    start = 0
    _annealed_sampling_processor = (
        lambda token_ids, logits: annealed_sampling_processor(
            token_ids,
            logits,
    ...
    sampling_param = SamplingParams(
        ...
        logits_processors=(
            [_annealed_sampling_processor] if args.use_annealed_sampling else None
        ),
    )
    if os.path.exists(checkpoint_filename):
        logliks, token_ids_num, output_texts, entropies = torch.load(
            checkpoint_filename
        )
    ...
    if args.ckpt_freq > 0:
        for j in range(start, len(prompts), args.ckpt_freq):
            ...
            inc_logliks, inc_token_ids_num, inc_output_texts, inc_entropies = (
                postprocessing_vllm_outputs(outputs, args)
            )
            ...
            torch.save(
                [logliks, token_ids_num, output_texts, entropies], checkpoint_filename
            )
            print(f"Saved Checkpoints at {j} / {len(prompts)} to {checkpoint_filename}")
    else:
        outputs = llm.generate(prompts[start:], sampling_param)
        inc_logliks, inc_token_ids_num, inc_output_texts, inc_entropies = (
            postprocessing_vllm_outputs(outputs, args)
        )
        logliks.extend(inc_logliks)
    ...
    print(
        f"LLM Generation Complete, Save Intermediate Results at {checkpoint_filename}"
    )
...
def postprocessing_vllm_outputs(vllm_outputs, args):
    ...
    entropies_w_token_ids = get_tokenwise_entropy_from_vllm_outputs(
        all_vllm_outputs, p=args.top_p, top_p_mode=True
    )
...
def main(data_name, args):
    ...
    output_metric_filename = out_file.replace(
        ".jsonl", f"_{args.prompt_type}_metrics.json"
    )
    checkpoint_filename = out_file.replace(".jsonl", ".ckpt")
    if (
        not args.overwrite
        and len(examples) == 0
        and os.path.exists(output_metric_filename)
    ):
        # no remained samples, just ends
        print(
            f"No remained examples and the user do not want to overwrite, report last time results for {data_name} wo llm initialization to save time"
        )
    ...
    for epoch in range(max_func_call):
        ...
        if args.use_vllm:
            logliks, token_ids_num, output_texts, entropies, outputs = run_llm_via_vllm(
                args, llm, prompts, stop_words, checkpoint_filename
            )
        else:
    ...
    for i, sample in enumerate(samples):
        ...
        token_ids_num_piece = token_ids_num[
        ...
        sample.update(
            {
                ...
                "pred": preds,
                "report": reports,
                "logliks": loglik_piece,
                "token_ids_num": token_ids_num_piece,
                "entropies": entropies_piece,
            }
        )
    ...
    with open(output_metric_filename, "w") as f:
